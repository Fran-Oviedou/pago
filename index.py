...
def home():
    ...
    if search_query:
        ...
        items = Producto.query.all()  # Carga todos los productos
...
def login():
    ...
    if form.validate_on_submit():
        # Usuario hardcodeado
        if form.usuario.data == "admin" and form.contraseña.data == "admin123":
            # usuario falso para testear
            usuario_hardcodeado = Usuario(
                id_usuario=1,
                user_usuario="admin",
                nombre_apellido="Admin admin",
                email="admin@ejemplo.com" 
            )
            login_user(usuario_hardcodeado)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        nuevo_usuario = Usuario(
            ...
            contraseña_usuario=form.contraseña.data,  # Guarda la contraseña sin hashear
        ...
        try:
            ...
            db.session.commit()
        except IntegrityError:  # error (usuario ya existe)
...
@app.route('/logout')
def logout():
    logout_user()  # Cierra la sesion
    return redirect(url_for('login'))  # Redirige a pagina del login
...
def añadir_carrito():
    ...
    #verificar si el producto esta en el carrito, añadir la cantidad exacta que se declaro, y añadirlo al carrito
...
def borrar_carrito():
    ...
    if carrito_prod_eliminado:  # Verifica que un producto exista
...
# Crear checkout,por donde se maneja la info del usuario y los productos para realizar el pago.
@app.route('/checkout', methods=["POST"])
@login_required
def checkout():
    # Obtener el id y el carrito no pagado del usuario que se encuentra en sesion.
    usuario_id = current_user.id_usuario
    carrito = Carrito.query.filter_by(id_usuario=usuario_id, status_carrito="no pagado").first()
